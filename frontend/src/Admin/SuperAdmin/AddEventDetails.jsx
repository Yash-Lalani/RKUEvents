...
const AddEventDetails = () => {
  ...
  useEffect(() => {
    setDepartments(["ALL", "SOE", "SOS", "SOM", "LAW", "PHARMACY"]);
  }, []);

  useEffect(() => {
    if (!selectedDepartment) return;
    const url = `http://localhost:5000/api/events?department=${selectedDepartment}`;
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        if (selectedDepartment === "ALL") {
          setEvents(data.filter(ev => (ev.departments || []).includes("ALL")));
        } else {
          setEvents(data);
        }
      })
    ...
    <div className="pb-10 min-h-[80vh] flex justify-center">
      <div className="w-full max-w-4xl">
        <div className="mb-8 text-center md:text-left">
          ...
          <p className="text-gray-400">Configure dynamic registration fields for specific events.</p>
        </div>

        <div className="glass rounded-3xl border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.3)] overflow-hidden">
          {/* Progress Header */}
          <div className="flex border-b border-white/10 bg-white/5">
            <div className={`flex-1 py-4 text-center font-semibold text-sm transition-colors ${step === 1 ? 'text-blue-400 border-b-2 border-blue-400 bg-blue-500/10' : 'text-gray-500'}`}>
              1. Select Target Event
            </div>
            <div className={`flex-1 py-4 text-center font-semibold text-sm transition-colors ${step === 2 ? 'text-blue-400 border-b-2 border-blue-400 bg-blue-500/10' : 'text-gray-500'}`}>
          ...
          <div className="p-8">
            <AnimatePresence mode="wait">
              {step === 1 && (
                ...
                >
                  <div>
                    <label className="block text-gray-400 text-sm font-medium mb-2">Target Department Filter</label>
                    <select
                    ...
                    >
                      ...
                      {departments.map(dep => (
                        <option key={dep} value={dep} className="bg-gray-900">{dep}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none appearance-none disabled:opacity-50"
                  ...
                  <div className="pt-6 flex justify-end">
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(99,102,241,0.3)] transition-all"
                    >
                ...
                >
                  <div className="space-y-4 mb-8">
                    {dynamicFields.map((field, i) => (
                      <div key={i} className="p-5 rounded-2xl bg-white/5 border border-white/10 relative group transition hover:border-blue-500/30">
                        <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                            />
                          </div>
                          <div>
                            ...
                            <select
                              ...
                              className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:ring-1 focus:ring-blue-500 outline-none appearance-none"
                            >
                        ...
                        {field.type === "select" && (
                          <div className="mt-4 pt-4 border-t border-white/10">
                            ...
                            <input
                              ...
                              className="w-full bg-black/40 border border-blue-500/30 rounded-lg px-3 py-2.5 text-blue-200 text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                            />
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-center pt-6 border-t border-white/10 gap-4">
                    <button
                      ...
                      className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-blue-500/30 text-blue-400 hover:bg-blue-500/10 font-medium transition w-full sm:w-auto justify-center"
                    >
