...
const EventRegisteredDepartmentUsers = () => {
  ...
  const currentDept = user?.department || user?.dept || "";
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        if (user?.role === "department-admin" && currentDept) {
  ...
  return (
    <div className="pb-10 min-h-[80vh]">
      <div className="mb-8">
        ...
        <p className="text-gray-400">View and export attendee records for {currentDept} events.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass p-6 rounded-3xl border border-red-500/30 bg-red-500/10 text-red-400 text-center">
          {err}
        </div>
      ) : (
              ...
              {events.length === 0 ? (
                ...
                events.map((event) => (
                  <div
                    ...
                    className="glass rounded-3xl overflow-hidden cursor-pointer hover:border-teal-500/50 transition-all duration-300 group shadow-[0_4px_30px_rgba(0,0,0,0.5)] border border-white/10 flex flex-col"
                  >
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4 z-20 flex flex-wrap gap-1 justify-end max-w-[80%]">
                        {(event.departments || []).map((dept, i) => (
                          <span key={i} className="px-2 py-1 text-[10px] font-bold uppercase rounded bg-teal-600/60 text-white backdrop-blur-md">
                            {dept}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="p-5 flex-1 flex flex-col justify-between">
                      <div className="space-y-3 mb-4">
                        <div className="flex items-center text-sm text-gray-400 gap-2">
                          <CalendarDaysIcon className="w-4 h-4 text-teal-400" />
              ...
              <div className="glass p-6 rounded-3xl border border-white/10 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={downloadCSV}
                    disabled={registrations.length === 0}
                    className="flex items-center gap-2 px-5 py-2.5 bg-teal-600/20 hover:bg-teal-600/40 text-teal-300 border border-teal-500/30 rounded-xl transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
